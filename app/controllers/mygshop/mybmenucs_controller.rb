class Mybshop::MybmenucsController < ApplicationController
  before_action :set_bmenuc, only: [:show, :edit, :update, :destroy]
  ...
  def show
    redirect_to mybshop_mybmenuc_mybmenus_path(mybmenuc_id:$bmenuc)
  end
  ...
  private
    ...
    def bmenuc_params
      params.require(:bmenuc).permit(:bshop_id, :shopname, :user_id, :name)
    end
end
